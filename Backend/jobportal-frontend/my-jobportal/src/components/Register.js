...
export default function Register() {
    ...
    const [serverErrors, setServerErrors] = useState(null) //to show the errors from d=server side to the client
    ...
    const runValidations = () => {
        ...
        if(email.trim().length == 0) { //This condition checks if the trimmed version of the email input has a
        errors.email = 'email is required'// length of zero, meaning it's empty or contains only whitespace. If so, it sets an error message indicating that the email is required.
    ...
    const handleSubmit = async (e) => {
        ...
        if(Object.keys(errors).length == 0) {
            try {
                ...
                setServerErrors(err.response.data.errors)  //if any of the field is empty it shows the error msg and these are defined in validation schema 
            }
        } else {
